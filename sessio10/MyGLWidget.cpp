...
void MyGLWidget::createBuffersTerraIParet ()
{
  ...
  glm::vec3 amb(0.2,0,0.2); //Com reacciona a la llum ambient
  glm::vec3 diff(0,0,1); //Com reacciona a la llum difusa
  glm::vec3 spec(0.8,0.8,0.8); //Com reacciona a reflexos (0,0,1) => només reacciona a la llum blava 
  float shin = 100;
...
void MyGLWidget::carregaShaders()
{
  ...
  colFocus = glGetUniformLocation (program->programId(), "colFocus2");
  posFocus = glGetUniformLocation (program->programId(), "posFocus2");
  llumAmbient = glGetUniformLocation (program->programId(), "llumAmbient2");
...
void MyGLWidget::modelTransformCamara (){
  ...
  glm::vec3 llum = glm::vec3(0.0,0.0,0.0);
