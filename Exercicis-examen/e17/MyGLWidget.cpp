...
void MyGLWidget::iniCamera ()
{
  ...
  OBS = glm::vec3(-2.0,1.5,0.0);
  VRP = glm::vec3(2.0,0.25,1.0);
  up = glm::vec3(0,1,0);
...
void MyGLWidget::tractarColorR(int color){
    makeCurrent();
    float aux = color/100.0f;
    diff.x = aux;
    createBuffersTerraIParet();
    update();
}
void MyGLWidget::tractarColorG(int color){
    makeCurrent();
    float aux = color/100.0f;
    diff.y = aux;
    createBuffersTerraIParet();
...
void MyGLWidget::tractarColorB(int color){
    makeCurrent();
    float aux = color/100.0f;
    diff.z = aux;
    createBuffersTerraIParet();    
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  glm::vec3 posicio(posx,-0.5,0.0);
  TG = glm::translate(TG, posicio);
  TG = glm::scale(TG, glm::vec3(escala2, escala2, escala2));
  double test = M_PI/2.0;
  TG = glm::rotate(TG, (float)test ,glm::vec3(0,1,0));
  TG = glm::translate(TG, -centrePatr);
