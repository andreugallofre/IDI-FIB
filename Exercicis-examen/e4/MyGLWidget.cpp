...
void MyGLWidget::designerZoom(int foverino) {
    makeCurrent();
    float fover = foverino/100.0f;
    FOV = fover;
    projectTransform();
    update();
}
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  glm::vec3 posicio(0.0,2.0,0.0);
  TG = glm::translate(TG, posicio);
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
  double test = M_PI;
  TG = glm::rotate(TG, (float)test ,glm::vec3(0,1,0));
  TG = glm::rotate(TG, (float)test ,glm::vec3(1,0,0));
  TG = glm::translate(TG, -centrePatr);
