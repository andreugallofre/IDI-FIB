...
void MyGLWidget::createBuffersCow ()
{
  ...
  // Creació del Vertex Array Object del Cowicio
...
void MyGLWidget::carregaShaders()
{
  ...
  lmao = glGetUniformLocation (program->programId(), "lmao");
}
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva)
                            //FOV           Aspect ratio    whatever
    Proj = glm::perspective(FOV, ra, (float)0.1, 3.0f*radiEsc);
  else
...
void MyGLWidget::calculaCapsaCow ()
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R: { // canvia òptica entre perspectiva i axonomètrica
      angleRotacio += (float)M_PI/6.0;
      modelTransformPatricio ();
      modelTransformPatricio2();
      centrexd = glm::vec3(1.0,-0.5,0.0);
      VRP = glm::vec3(centrexd.x * cos(-angleRotacio) + centrexd.z * sin(-angleRotacio) , centrexd.y, -centrexd.x * sin(-angleRotacio) + centrexd.z * cos(-angleRotacio));
      viewTransform();
